...
The build script we'll be using is contained in the ALNJ github repository ([contrib/gitian-build.py](https://github.com/alnj-project/alnj/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the ALNJ repository and will need to fetch this script with the following commands:
...
- Clone the gitian-builder, gitian.sigs, alnj-detached-sigs, and alnj GitHub repos.
