...
The build script we'll be using is contained in the ALNJ github repository ([contrib/gitian-build.py](https://github.com/alnjl-project/alnjl/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the ALNJ repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/ALNJ-Project/ALNJ/master/contrib/gitian-build.py
...
- Clone the gitian-builder, gitian.sigs, alnjl-detached-sigs, and alnjl GitHub repos.
