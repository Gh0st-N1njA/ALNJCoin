...
The build script we'll be using is contained in the PCTM github repository ([contrib/gitian-build.py](https://github.com/Gh0st-N1njA/PactumCoin/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the PCTM repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/PCTM-Project/PCTM/master/contrib/gitian-build.py
