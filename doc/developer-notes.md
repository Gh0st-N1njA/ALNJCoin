...
- [Development guidelines](#development-guidelines)
    - [General ALNJ Core](#general-alnjl-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Run with the `-testnet` option to run with "play PIVs (tALNJ)" on the test network, if you
...
([`valgrind.supp`](https://github.com/alnjl-project/alnjl/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_alnjl
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_alnjl --log_level=test_suite
$ valgrind -v --leak-check=full src/alnjld -printtoconsole
```
...
# A coverage report will now be accessible at `./test_alnjl.coverage/index.html`.
```
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `alnjl-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `alnjl-cli` and the GUI debug console use this table to determine how to
