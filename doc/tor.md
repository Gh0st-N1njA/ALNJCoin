...
It is possible to run PCTM Core as a Tor hidden service, and connect to such services.

The following directions assume you have a Tor proxy running on port 9050. Many distributions default to having a SOCKS proxy listening on port 9050, but others may not. In particular, the Tor Browser Bundle defaults to listening on port 9150. See [Tor Project FAQ:TBBSocksPort](https://www.torproject.org/docs/faq.html.en#TBBSocksPort) for how to properly
configure Tor.


## 1. Run PCTM Core behind a Tor proxy
----------------------------------

The first step is running PCTM behind a Tor proxy. This will already anonymize all
...
your pactumcoind's P2P listen port (12122 by default).
...
PCTM Core has been updated to make use of this.

This means that if Tor is running (and proper authentication has been configured),
PCTM Core automatically creates a hidden service to listen on. This will positively
affect the number of available .onion nodes.

This new feature is enabled by default if PCTM Core is listening (`-listen`), and
...
in `torrc` config file. For cookie authentication the user running pactumcoind must have read
access to the `CookieAuthFile` specified in Tor configuration. In some cases this is
preconfigured and the creation of a hidden service is automatic. If permission problems
are seen with `-debug=tor` they can be resolved by adding both the user running Tor and
the user running pactumcoind to the same group and setting permissions appropriately. On
Debian-based systems the user running pactumcoind can be added to the debian-tor group,
...
- Do not add anything but PCTM Core ports to the hidden service created in section 2.
