UNIX BUILD NOTES
====================
Some notes on how to build ALNJ Core in Unix.

Note
---------------------
Always use absolute paths to configure and compile ALNJ Core and the dependencies,
...
This will build alnj-qt as well, if the dependencies are met.
...
memory available when compiling ALNJ Core. On systems with less, gcc can be
 ...
 **For Ubuntu only:** db4.8 packages are available [here](https://launchpad.net/~alnj/+archive/alnj).
 You can add the repository using the following command:

    sudo apt-get install software-properties-common
    sudo add-apt-repository ppa:alnj/alnj
    sudo apt-get update
...
To build ALNJ Core without wallet, see [*Disable-wallet mode*](/doc/build-unix.md#disable-wallet-mode)
...
If you want to build alnj-qt, make sure that the required packages for Qt development
...
Once these are installed, they will be found by configure and a alnj-qt executable will be
built by default.
...
Notes
-----
The release is built with GCC and then "strip alnjd" to strip the debug
...
Security
--------
To help make your ALNJ Core installation more secure by making certain attacks impossible to
...
* _Non-executable Stack_: If the stack is executable then trivial stack-based buffer overflow exploits are possible if
    vulnerable buffers are found. By default, ALNJ Core should be built with a non-executable stack
