...
ALNJ Core version *version* is now available from:  <https://github.com/alnj-project/alnj/releases>
...
The p2p alert system has been removed in [PR #1372](https://github.com/ALNJ-Project/ALNJ/pull/1372) and the 'alert' message is no longer supported.

### GUI

**Keyboard navigation**: dialogs can now be accepted with the `ENTER` (`RETURN`) key, and dismissed with the `ESC` key ([#1392](https://github.com/ALNJ-Project/ALNJ/pull/1392)).
...
**Custom Fee**: The custom fee selected when sending a transaction is now saved in the wallet database and persisted across multiple sends and wallet's restarts ([#1406](https://github.com/ALNJ-Project/ALNJ/pull/1406)).


**Include delegations in send**: The send and cold-staking page present a checkbox ([#1391](https://github.com/ALNJ-Project/ALNJ/pull/1391)) to make the automatic input selection algorithm include delegated (P2CS) utxos if needed. The option is unchecked by default.


**Staking Charts**: can now be hidden at startup (with a flag `--hidecharts`) or at runtime with a checkbox in settings --> options --> display ([PR #1475](https://github.com/ALNJ-Project/ALNJ/pull/1475)).
...
__Context Lock/Unlock__ [[PR #1387](https://github.com/ALNJ-Project/ALNJ/pull/1387)]:<br>
...
- The `-reservebalance` configuration/startup option has been removed ([PR #1373](https://github.com/ALNJ-Project/ALNJ/pull/1373)).
