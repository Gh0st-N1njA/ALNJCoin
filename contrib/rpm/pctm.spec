...
cp -p %{SOURCE10} ./pctm.conf.example
tar -zxf %{SOURCE1}
...
mv %{buildroot}%{_bindir}/pactumcoind %{buildroot}%{_sbindir}/pactumcoind
...
# man pages
install -D -p %{SOURCE20} %{buildroot}%{_mandir}/man1/pactumcoind.1
install -p %{SOURCE21} %{buildroot}%{_mandir}/man1/pactumcoin-cli.1
%if %{_buildqt}
install -p %{SOURCE22} %{buildroot}%{_mandir}/man1/pactumcoin-qt.1
%endif
...
%{_sbindir}/semanage port -a -t pctm_port_t -p tcp 18332
...
%attr(0644,root,root) %{_mandir}/man1/pactumcoin-qt.1*
%endif
...
%defattr(-,root,root,-)
%license COPYING
%doc COPYING pctm.conf.example doc/README.md
