...
pkgdatadir = $(datadir)/alnj
pkgincludedir = $(includedir)/alnj
pkglibdir = $(libdir)/alnj
pkglibexecdir = $(libexecdir)/alnj
...
CONFIG_HEADER = $(top_builddir)/src/config/alnj-config.h
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	...
	$(top_srcdir)/src/config/alnj-config.h.in \
...
ACLOCAL = ${SHELL} /root/ALNJCoin/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
ARFLAGS = cr
AUTOCONF = ${SHELL} /root/ALNJCoin/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /root/ALNJCoin/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /root/ALNJCoin/build-aux/missing automake-1.15
AVX2_CXXFLAGS = -mavx -mavx2
AWK = mawk
BDB_CFLAGS = 
BDB_CPPFLAGS = 
BDB_LIBS = -ldb_cxx
BITCOIN_CLI_NAME = alnj-cli
BITCOIN_DAEMON_NAME = alnjd
BITCOIN_GUI_NAME = alnj-qt
BITCOIN_TX_NAME = alnj-tx
...
LUPDATE = 
MAINT = 
MAKEINFO = ${SHELL} /root/ALNJCoin/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
MINIUPNPC_LIBS = 
...
OTOOL64 = 
PACKAGE = alnj
PACKAGE_BUGREPORT = https://github.com/alnj-project/alnj/issues
PACKAGE_NAME = ALNJ Core
PACKAGE_STRING = ALNJ Core 0.1.1
PACKAGE_TARNAME = alnj
PACKAGE_URL = https://algoninja.site/alnj/
...
ZMQ_CFLAGS = 
ZMQ_LIBS = 
abs_builddir = /root/ALNJCoin
abs_srcdir = /root/ALNJCoin
abs_top_builddir = /root/ALNJCoin
abs_top_srcdir = /root/ALNJCoin
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /root/ALNJCoin/build-aux/install-sh
...
DIST_CONTRIB = $(top_srcdir)/contrib/alnj-cli.bash-completion \
	       $(top_srcdir)/contrib/alnj-tx.bash-completion \
	       $(top_srcdir)/contrib/alnjd.bash-completion \
...
WINDOWS_PACKAGING = $(top_srcdir)/share/pixmaps/alnj.ico \
  $(top_srcdir)/share/pixmaps/nsis-header.bmp \
  $(top_srcdir)/share/pixmaps/nsis-wizard.bmp \
  $(top_srcdir)/doc/README_windows.txt

LINUX_PACKAGING = $(top_srcdir)/share/pixmaps/alnj16.xpm \
  $(top_srcdir)/share/pixmaps/alnj32.xpm \
  $(top_srcdir)/share/pixmaps/alnj128.png
...
COVERAGE_INFO = baseline.info \
  test_alnj_filtered.info total_coverage.info \
  baseline_filtered.info functional_test.info functional_test_filtered.info \
  test_alnj_coverage.info test_alnj.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/ALNJ-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/alnj-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/alnj-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/alnj-config.h
$(top_srcdir)/src/config/alnj-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/alnj-config.h src/config/stamp-h1
...
$(OSX_APP)/Contents/MacOS/ALNJ-Qt: all-recursive
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/ALNJ-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/ALNJ-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_alnj.info: baseline_filtered.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c $(LCOV_OPTS) -d $(abs_builddir)/src -t test_alnj -o $@
#	$(LCOV) -z $(LCOV_OPTS) -d $(abs_builddir)/src

#test_alnj_filtered.info: test_alnj.info
#	$(abs_builddir)/contrib/filter-lcov.py $(LCOV_FILTER_PATTERN) $< $@
#	$(LCOV) -a $@ $(LCOV_OPTS) -o $@

#functional_test.info: test_alnj_filtered.info
...
#test_alnj_coverage.info: baseline_filtered.info test_alnj_filtered.info
#	$(LCOV) -a $(LCOV_OPTS) baseline_filtered.info -a test_alnj_filtered.info -o $@

#total_coverage.info: test_alnj_filtered.info functional_test_filtered.info
#	$(LCOV) -a $(LCOV_OPTS) baseline_filtered.info -a test_alnj_filtered.info -a functional_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_alnj.coverage/.dirstamp:  test_alnj_coverage.info
...
#cov: test_alnj.coverage/.dirstamp total.coverage/.dirstamp
...
clean-local: clean-docs
	rm -rf coverage_percent.txt test_alnj.coverage/ total.coverage/ test/tmp/ cache/ $(OSX_APP)
