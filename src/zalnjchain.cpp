...
{
    CDataStream serializedCoinSpend = ZALNJModule::ScriptSigToSerializedSpend(txin.scriptSig);
...
bool UpdateZALNJSupplyConnect(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    AssertLockHeld(cs_main);
    ...
    //Add mints to zALNJ supply (mints are forever disabled after last checkpoint)
    ...
    //Remove spends from zALNJ supply
    ...
    // A one-time event where only the zALNJ supply was off (due to serial duplication off-chain on main net)
...
bool UpdateZALNJSupplyDisconnect(const CBlock& block, CBlockIndex* pindex)
{
    AssertLockHeld(cs_main);
    ...
    // Re-add spends to zALNJ supply
    std::list<libzerocoin::CoinDenomination> listDenomsSpent = ZerocoinSpendListFromBlock(block, true);
    for (const libzerocoin::CoinDenomination& denom : listDenomsSpent) {
        mapZerocoinSupply.at(denom)++;
    }

    // Remove mints from zALNJ supply (mints are forever disabled after last checkpoint)
    if (pindex->nHeight < consensus.height_last_ZC_AccumCheckpoint) {
        std::list<CZerocoinMint> listMints;
        std::set<uint256> setAddedToWallet;
        BlockToZerocoinMintList(block, listMints, true);
