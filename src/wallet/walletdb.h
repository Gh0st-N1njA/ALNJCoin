...
public:
    ...
    bool WriteZALNJSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZALNJSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZALNJSeed_deprecated(uint256& seed);
    bool EraseZALNJSeed();
    bool EraseZALNJSeed_deprecated();

    bool WriteZALNJCount(const uint32_t& nCount);
    bool ReadZALNJCount(uint32_t& nCount);
