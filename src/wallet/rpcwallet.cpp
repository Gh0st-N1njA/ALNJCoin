...
UniValue getaccountaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            ...
            "\nReturns the current ALNJ address for receiving payments to this account.\n"
...
UniValue multisend(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 2)
        throw std::runtime_error(
            ...
            "multisend <ALNJ Address> <percent>\n"
...
UniValue spendzerocoin(const UniValue& params, bool fHelp)
{
    ...
    return DoZalnjSpend(nAmount, fMintChange, fMinimizeChange, vMintsSelected, address_str, ispublicspend);
}
...
UniValue spendzerocoinmints(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
            ...
            "\nSpend zPIV mints to a ALNJ address.\n" +
            HelpRequiringPassphrase() + "\n"
    ...
    return DoZalnjSpend(nAmount, false, true, vMintsSelected, address_str);
}


extern UniValue DoZalnjSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, std::vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool ispublicspend)
{
...
UniValue resetmintzerocoin(const UniValue& params, bool fHelp)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    CzPIVTracker* zalnjTracker = pwalletMain->zalnjTracker.get();
...
UniValue createautomintaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
                ...
                "\nResult\n"
                "\"address\"     (string) ALNJ address for auto minting\n" +
...
UniValue spendrawzerocoin(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 4 || params.size() > 5)
        throw std::runtime_error(
            ...
            "5. \"address\"          (string, optional) ALNJ address to spend to. If not specified, spend to change add.\n"
...
UniValue clearspendcache(const UniValue& params, bool fHelp)
{
...

    {
        int nTries = 0;
        while (nTries < 100) {
            TRY_LOCK(zalnjTracker->cs_spendcache, fLocked);
            if (fLocked) {
                if (zalnjTracker->ClearSpendCache()) {
