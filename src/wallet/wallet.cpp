...
/**
 * Fees smaller than this (in ualng) are considered zero fee (for transaction creation)
        ...
        if (zwalletMain) {
            ...
            if (CWalletDB(strWalletFile).ReadCurrentSeedHash(hashSeed)) {
                ...
                if (!GetDeterministicSeed(hashSeed, nSeed)) {
                    return error("Failed to read zALNJ seed from DB. Wallet is probably corrupt.");
                }
...
int CWallet::ScanForWalletTransactions(CBlockIndex* pindexStart, bool fUpdate, bool fromStartup)
{
    ...
    bool fCheckZALNJ = GetBoolArg("-zapwallettxes", false);
    if (fCheckZALNJ)
        zalnjTracker->Init();
    ...
    CBlockIndex* pindex = pindexStart;
    {
        ...
        while (pindex) {
            ...
            if (fCheckZALNJ && pindex->nHeight >= Params().GetConsensus().height_start_ZC) {
        ...
        )
{
    ...
    // Parse utxos into CAlngStakes
    std::list<std::unique_ptr<CStakeInput> > listInputs;
    for (const COutput &out : vCoins) {
        std::unique_ptr<CAlngStake> input(new CAlngStake());
