...
/**
 * Fees smaller than this (in ualnj) are considered zero fee (for transaction creation)
...
int CWallet::ScanForWalletTransactions(CBlockIndex* pindexStart, bool fUpdate, bool fromStartup)
{
    ...
    CBlockIndex* pindex = pindexStart;
    {
        ...
        while (pindex) {
            ...
            //If this is a zapwallettx, need to readd zalnj
        ...
        int blockHeight, bool fPrecompute)
{
    ...
    if (GetBoolArg("-alnjstake", true) && !fPrecompute) {
    ...
    //zPIV
    if ((GetBoolArg("-zalnjstake", true) || fPrecompute) && chainActive.Height() > Params().Zerocoin_Block_V2_Start() && !sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ...
        std::set<CMintMeta> setMints = zalnjTracker->ListMints(true, true, fUpdate);
...
bool CWallet::MintableCoins()
{
    ...
    CAmount nZalnjBalance = GetZerocoinBalance(false);
...
bool CWallet::GetBudgetSystemCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    vecSend.push_back(std::make_pair(scriptChange, BUDGET_FEE_TX_OLD)); // Old 50 ALNJ collateral
...
bool CWallet::GetBudgetFinalizationCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    std::string strFail = "";
    std::vector<std::pair<CScript, CAmount> > vecSend;
    vecSend.push_back(std::make_pair(scriptChange, BUDGET_FEE_TX)); // New 5 ALNJ collateral
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX, false, fIncludeDelegated)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 10000 ALNJ.");
                    } else if (coin_type == ONLY_NONDENOMINATED_NOT10000IFMN) {
                        strFailReason = _("Unable to locate enough Obfuscation non-denominated funds for this transaction that are not equal 10000 ALNJ.");
                    } else {
...
void CWallet::AutoZeromint()
{
    ...
    // zPIV are integers > 0, so we can't mint 10% of 9 ALNJ
    if (nBalance < 10){
    ...
    // If a preferred denomination is used (means nPreferredDenom != 0) do nothing until we have enough ALNJ to mint this denomination
                         ...
                         CZerocoinSpendReceipt& receipt, libzerocoin::SpendType spendType, CBlockIndex* pindexCheckpoint)
{
    ...
    while (nLockAttempts < 100) {
        TRY_LOCK(zalnjTracker->cs_spendcache, lockSpendcache);
        if (!lockSpendcache) {
        ...
        for (auto &it : mapMintsSelected) {
            ...
            CMintMeta meta = zalnjTracker->Get(GetSerialHash(mint.GetSerialNumber()));
            CoinWitnessData *coinWitness = zalnjTracker->GetSpendCache(meta.hashStake);
...
std::string CWallet::GetUniqueWalletBackupName(bool fzalnjAuto) const
{
    ...
    return strprintf("wallet%s.dat%s", fzalnjAuto ? "-autozalnjbackup" : "", DateTimeStrFormat(".%Y-%m-%d-%H-%M", GetTime()));
}
...
void CWallet::ZPivBackupWallet()
{
    ...
    for (int i = 0; i < 10; i++) {
        strNewBackupName = strprintf("wallet-autozalnjbackup-%d.dat", i);
        ...
        if (boost::filesystem::exists(backupPath)) {
            ...
            if (i <= 8) {
                ...
                boost::filesystem::path nextBackupPath = backupDir / strprintf("wallet-autozalnjbackup-%d.dat", i + 1);
            ...
            strNewBackupName = strprintf("wallet-autozalnjbackup-%d.dat", 0);
            backupPath = backupDir / strNewBackupName;
            break;
        }
    ...
    if(!GetArg("-zalnjbackuppath", "").empty()) {
        boost::filesystem::path customPath(GetArg("-zalnjbackuppath", ""));
    ...
    while (true) {
        ...
        for (std::unique_ptr <CStakeInput>& stakeInput : listInputs) {
...

            {
                ...
                CoinWitnessData* witnessData = zalnjTracker->GetSpendCache(serialHash);
...
{
    zwalletMain = zwallet;
    zalnjTracker = std::unique_ptr<CzPIVTracker>(new CzPIVTracker(strWalletFile));
}
