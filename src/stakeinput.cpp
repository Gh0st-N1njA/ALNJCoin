...
bool CAlngStake::InitFromTxIn(const CTxIn& txin)
{
    if (txin.IsZerocoinSpend())
        return error("%s: unable to initialize CAlngStake from zerocoin spend");
...
bool CAlngStake::SetPrevout(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CAlngStake::GetTxFrom(CTransaction& tx) const
{
    if (txFrom.IsNull())
        return false;
    tx = txFrom;
...
bool CAlngStake::GetTxOutFrom(CTxOut& out) const
{
...
bool CAlngStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CAlngStake::GetValue() const
{
    return txFrom.vout[nPosition].nValue;
}

bool CAlngStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
CDataStream CAlngStake::GetUniqueness() const
{
    //The unique identifier for a ALNJ stake is the outpoint
...
CBlockIndex* CAlngStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CAlngStake::ContextCheck(int nHeight, uint32_t nTime)
{
