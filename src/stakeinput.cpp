...
{
    CzPIVTracker* zalnjTracker = pwallet->zalnjTracker.get();
    CMintMeta meta;
    if (!zalnjTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zalnjTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
