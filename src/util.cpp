...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\PCTM
// Windows >= Vista: C:\Users\Username\AppData\Roaming\PCTM
// Mac: ~/Library/Application Support/PCTM
// Unix: ~/.pctm
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "PCTM";
#else
    fs::path pathRet;
...
{
    fs::path pathConfigFile(GetArg("-conf", "pctm.conf"));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty pctm.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override pctm.conf
...
fs::path GetPidFile()
{
    fs::path pathPidFile(GetArg("-pid", "pactumcoind.pid"));
