...
void AskPassphraseDialog::accept()
{
    ...
    case Mode::Encrypt: {
        ...
        bool ret = openStandardDialog(
                ...
                " <b>" + tr("LOSE ALL OF YOUR PCTM") + "</b>!<br><br>" + tr("Are you sure you wish to encrypt your wallet?"),
                tr("ENCRYPT"), tr("CANCEL")
        );
        if (ret) {
            if (newpass1 == newpass2) {
                newpassCache = newpass1;
                PCTMGUI* window = static_cast<PCTMGUI*>(parentWidget());
...
{
    PCTMGUI* gui = static_cast<PCTMGUI*>(parentWidget());
...
{
    hide();
    static_cast<PCTMGUI*>(parentWidget())->showHide(true);
    openStandardDialog(
            tr("Wallet encrypted"),
            "<qt>" +
            tr("PCTM will close now to finish the encryption process. "
               "Remember that encrypting your wallet cannot fully protect "
               "your PCTMs from being stolen by malware infecting your computer.") +
            "<br><br><b>" +
