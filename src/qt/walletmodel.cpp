...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl, bool fIncludeDelegations)
{
...

    {
        ...
        if (recipients[0].useSwiftTX && total > sporkManager.GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            Q_EMIT message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 ALNJ.").arg(sporkManager.GetSporkValue(SPORK_5_MAX_VALUE)),
...
bool WalletModel::createZalngSpend(
        CWalletTx &wtxNew,
...
bool WalletModel::convertBackZalng(
        CAmount value,
        std::vector<CZerocoinMint> &vMintsSelected,
        CZerocoinSpendReceipt &receipt)
{
...
static void NotifyzALNJReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzALNJReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzALNJReset.connect(boost::bind(NotifyzALNJReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzALNJReset.disconnect(boost::bind(NotifyzALNJReset, this));
...
bool WalletModel::updateAddressBookPurpose(const QString &addressStr, const std::string& purpose)
{
    ...
    if (address.IsStakingAddress())
        return error("Invalid ALNJ address, cold staking address");
    CKeyID keyID;
...
{
    if (!address.IsValid())
        return error("Invalid ALNJ address");

    if (!address.GetKeyID(keyID))
        return error("Unable to get KeyID from ALNJ address");
...
{
    setMints.clear();
    setMints = pwalletMain->zalnjTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus, fWrongSeed);
}
