...
#include "qt/alnjl/forms/ui_send.h"
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (ALNJ) or private coins (zPIV)"));
    ...
    setCssProperty(coinIcon, "coin-icon-alnj");
...
void SendWidget::refreshAmounts() {
    ...
    bool isZalnj = ui->pushRight->isChecked();
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();

    ui->labelAmountSend->setText(GUIUtil::formatBalance(total, nDisplayUnit, isZalnj));
    ...
    if (CoinControlDialog::coinControl->HasSelected()){
        ...
        // Wallet's balance
        totalAmount = (isZalnj ? walletModel->getZerocoinBalance() : walletModel->getBalance()) - total;
...
void SendWidget::onSendClicked(){
    ...
    if((sendPiv) ? send(recipients) : sendZalnj(recipients)) {
...
bool SendWidget::sendZalnj(QList<SendCoinsRecipient> recipients){
    if (!walletModel || !walletModel->getOptionsModel())
        return false;
...
void SendWidget::onPIVSelected(bool _isPIV){
    isPIV = _isPIV;
    setCssProperty(coinIcon, _isPIV ? "coin-icon-alnj" : "coin-icon-zalnj");
    refreshView();
...
void SendWidget::onContactMultiClicked(){
    if(focusedEntry) {
        ...
        CBitcoinAddress alnjAdd = CBitcoinAddress(address.toStdString());
        if (walletModel->isMine(alnjAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(alnjAdd.Get(), dialog->getLabel().toStdString(), "send")) {
