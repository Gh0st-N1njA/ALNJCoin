...
#include "qt/alnj/forms/ui_snackbar.h"
#include "qt/alnj/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(ALNJGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &ALNJGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
