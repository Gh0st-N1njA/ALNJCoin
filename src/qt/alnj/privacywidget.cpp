    ...
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zPIV anonymizes your ALNJ by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zPIV or convert back to ALNJ"));
    ...
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select ALNJ outputs to mint into zPIV.");
    ...
    connect(ui->btnTotalzPIV, SIGNAL(clicked()), this, SLOT(onTotalZalnjClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    ...
    if(isMint){
        ...
        ui->labelSubtitleAmount->setText(tr("Enter amount of ALNJ to mint into zPIV"));
    }else{
        btnText = tr("Convert back to ALNJ");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zPIV to convert back into ALNJ"));
    }
...
void PrivacyWidget::onTotalZalnjClicked(){
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZalnj(
            value,
            selectedMints,
            mintChange,
            minimizeChange,
        ...
        // Spend succeed
        inform(tr("zPIV converted back to ALNJ"));
        // clear
        ui->lineEditAmount->clear();
    }
}
