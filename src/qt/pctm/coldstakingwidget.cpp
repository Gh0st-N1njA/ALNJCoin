...
ColdStakingWidget::ColdStakingWidget(PCTMGUI* parent) :
    PWidget(parent),
    ...
    isLoading(false)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can delegate your PCTMs, letting a hot node (24/7 online node)\nstake on your behalf, while you keep the keys securely offline."));
    ...
    ui->labelSubtitle2->setText(tr("Accept PCTM delegation / Delegate PCTM"));
    ...
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select PCTM outputs to delegate.");
...
void ColdStakingWidget::updateStakingTotalLabel()
{
    ...
    ui->labelStakingTotal->setText(tr("Total Staking: %1").arg(
            (total == 0) ? "0.00 PCTM" : GUIUtil::formatBalance(total, nDisplayUnit))
    );
}
