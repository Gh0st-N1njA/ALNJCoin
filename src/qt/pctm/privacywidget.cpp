...
#include "qt/pctm/forms/ui_privacywidget.h"
#include "qt/pctm/qtutils.h"
#include "guiutil.h"
#include "qt/pctm/txviewholder.h"
...
PrivacyWidget::PrivacyWidget(PCTMGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zPCTM");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zPCTM");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zPCTM");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zPCTM");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zPCTM");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zPCTM");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zPCTM");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zPCTM");
    setCssProperty(ui->labelValueDenom5000, "text-body2");

    ui->layoutDenom->setVisible(false);

    // List
    ui->labelListHistory->setText(tr("Last zPCTM Movements"));
    ...
    ui->btnTotalzPCTM->setTitleClassAndText("btn-title-grey", "Total 0 zPCTM");
    ui->btnTotalzPCTM->setSubTitleClassAndText("text-subtitle", "Show denominations of zPCTM owned.");
    ...
    ui->btnResetZerocoin->setTitleClassAndText("btn-title-grey", "Reset Spent zPCTM");
    ui->btnResetZerocoin->setSubTitleClassAndText("text-subtitle", "Reset zerocoin database.");

    connect(ui->btnTotalzPCTM, &OptionButton::clicked, this, &PrivacyWidget::onTotalZpivClicked);
...
void PrivacyWidget::onTotalZpivClicked()
{
    ...
    if (!isVisible) {
        ...
        ui->btnTotalzPCTM->setRightIconClass("btn-dropdown", true);
    } else {
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzPCTM->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked()
{
    ...
    if (sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zPCTM is currently undergoing maintenance"));
        return;
    }
    ...
    if (!ctx.isValid()) {
        inform(tr("You need to unlock the wallet to be able to %1 zPCTM").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value)
{
    ...
    if (!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)) {
        ...
        // Mint succeed
        inform(tr("zPCTM minted successfully"));
        // clear
...
void PrivacyWidget::updateDenomsSupply()
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zPCTM </b>";
    ...
    ui->btnTotalzPCTM->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
