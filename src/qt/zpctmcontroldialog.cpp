...
#include "ui_zpctmcontroldialog.h"
    ...
    model(0)
{
    ...
    // Title
    ui->labelTitle->setText(tr("Select zPCTM Denominations to Spend"));
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < nRequiredConfs)
                ...
                strReason = "Your wallet is locked. Impossible to spend zPCTM.";
            else if (!mint.isSeedCorrect)
                strReason = "The zPCTM seed used to mint this zPCTM is not the same as currently hold in the wallet";
            else
