...
#include "lightzalnjthread.h"
#include "main.h"

/****** Thread ********/
void CLightWorker::ThreadLightZPIVSimplified() {
    RenameThread("alnjl-light-thread");
    ...
    while (true) {
        try {
            ...
            LogPrintf("%s pop work for %s \n\n", "alnjl-light-thread", genWit.toString());
            ...
            if (!pIndex) {
                ...
                if (blockHeight >= Params().Zerocoin_Block_V2_Start()) {
                    ...
                    if (!res) {
                        ...
                        ss << heightStop;
                        if (genWit.getPfrom()) {
                            LogPrintf("%s pushing message to %s \n", "alnjl-light-thread", genWit.getPfrom()->addrName);
            ...
            PrintExceptionContinue(&e, "lightzalnjthread");
            break;
        }
    }
...
void CLightWorker::rejectWork(CGenWit& wit, int blockHeight, uint32_t errorNumber) {
    if (wit.getStartingHeight() == blockHeight){
        LogPrintf("%s rejecting work %s , error code: %s\n", "alnjl-light-thread", wit.toString(), errorNumber);
        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
