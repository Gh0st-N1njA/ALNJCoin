...
bool CzPCTMWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZPCTMCount(nCountLastUsed);
    else if (!walletdb.ReadZPCTMCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzPCTMWallet::Lock()
{
    seedMaster.SetNull();
}

void CzPCTMWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzPCTMWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZPCTM(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzPCTMWallet::LoadMintPoolFromDB()
{
...
void CzPCTMWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzPCTMWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzPCTMWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzPCTMWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZPCTM(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzPCTMWallet::SeedToZPCTM(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
    libzerocoin::ZerocoinParams* params = Params().GetConsensus().Zerocoin_Params(false);
...
uint512 CzPCTMWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzPCTMWallet::GenerateDeterministicZPCTM(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzPCTMWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzPCTMWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzPCTMWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
