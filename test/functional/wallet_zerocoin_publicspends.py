...
class ZerocoinSpendTest(AlnjTestFramework):
    ...
    def run_test(self):
        ...
        def check_balances(denom, zalnj_bal, alng_bal):
            zalnj_bal -= denom
            assert_equal(self.nodes[2].getzerocoinbalance()['Total'], zalnj_bal)
            alng_bal += denom
            wi = self.nodes[2].getwalletinfo()
            assert_equal(wi['balance'] + wi['immature_balance'], alng_bal)
            return zalnj_bal, alng_bal
        ...
        zalnj_balance = self.nodes[2].getzerocoinbalance()['Total']
        assert_equal(balance, DecimalAmt(13833.92))
        assert_equal(zalnj_balance, 6666)
        ...
        zalnj_balance, balance = check_balances(denom_2, zalnj_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zalnj_balance, balance = check_balances(denom_3, zalnj_balance, balance)
        ...
        _, _ = check_balances(denom_1, zalnj_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v3) PASSED")
